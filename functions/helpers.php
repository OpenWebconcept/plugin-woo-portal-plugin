...
if ( ! function_exists( 'woo_portal_get_url' ) ) {
	 ...
	 */
	function woo_portal_get_url( $value = '' ) {
		...
		if ( is_array( $value ) ) {
			...
			if ( ! empty( $value['url'] ) ) {
				return woo_portal_get_url( $value['url'] );
			}

			// Return url, if not already returned permalink or archive link.
			if ( ! empty( $value['title'] ) && wp_http_validate_url( $value['title'] ) ) {
				return woo_portal_get_url( $value['title'] );
			}
		}
...
if ( ! function_exists( 'woo_portal_is_external_url' ) ) {
	 ...
	 * @return bool
	 */
	function woo_portal_is_external_url( $input ) {

		if ( empty( $input ) ) {
			return false;
		}

		$woo_portal_home_url = defined( 'WP_HOME' ) ? WP_HOME : home_url();

		// Check if domain is within the provided string.
		if ( is_string( $input ) ) {
			if ( wp_http_validate_url( $input ) ) {
				return false === strpos( untrailingslashit( $input ), untrailingslashit( $woo_portal_home_url ) );
			}
...
if ( ! function_exists( 'woo_portal_get_file_size' ) ) {
	 ...
	 * @return string
	 */
	function woo_portal_get_file_size( $attachment_id ) {
		...
		$woo_portal_file = get_attached_file( $attachment_id );

		if ( empty( $woo_portal_file ) || ! file_exists( $woo_portal_file ) || is_wp_error( $woo_portal_file ) ) {
			// File can't be reached, probably the file isn't in the uploads directory.
			return '0 B';
		}

		$woo_portal_bytes = filesize( $woo_portal_file );
		...
		$woo_portal_sizes            = [ 'B', 'KB', 'MB', 'GB', 'TB', 'PB', 'EB', 'ZB', 'YB' ];
		$woo_portal_size_calculation = floor( log( $woo_portal_bytes ) / log( 1024 ) );

		return sprintf( '%s %s', number_format_i18n( $woo_portal_bytes / pow( 1024, floor( $woo_portal_size_calculation ) ), 1 ), $woo_portal_sizes[ $woo_portal_size_calculation ] );
	}
}

if ( ! function_exists( 'woo_portal_file_get_contents' ) ) {
	/**
	 ...
	 * @return string
	 */
	function woo_portal_file_get_contents( $path_or_url ) {
